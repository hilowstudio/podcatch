...
export async function updateNotionSettings(accessToken: string, pageId: string) {
    const session = await auth();
    ...
    try {
        await prisma.user.update({
            where: { id: session.user.id },
            data: {
                notionAccessToken: accessToken || null,
        ...
        console.error('Failed to update Notion settings:', error);
        return { success: false, error: 'Failed to update settings' };
    }
}

        ...
        for (const feed of feeds) {
            await step.run(`process-feed-${feed.id}`, async () => {
                ...
                try {
                    ...
                    if (feedUrl.includes('podcasts.apple.com')) {
                        const match = feedUrl.match(/id(\d+)/);
                        if (match) {
                            const itunesId = match[1];
                            console.log(`Detected Apple Podcast URL for ${feed.id}, looking up ID: ${itunesId}`);
                            ...
                            try {
                                ...
                                if (data.results && data.results.length > 0 && data.results[0].feedUrl) {
                                    const newUrl = data.results[0].feedUrl;
                                    console.log(`Resolved Apple URL to RSS: ${newUrl}`);

                                    // Update DB with correct RSS URL
                                ...
                                console.error('Failed to resolve Apple ID:', err);
                            }
                        }
                    }
                    ...
                    for (const item of rssFeed.items) {
                        ...
                        // Episodes are discovered only. Processing is manual
                        // (user-initiated) — there is no auto-processing.
                    }
